...
// Reine Read-only-Darstellung eines Tickets (Sidebar + Inhalt + Verlauf +
// Nachträge). KEINE Admin-Aktionen – die liegen im Admin-Detail-View.
...
// ── Nachträge (aus dem Verlauf extrahiert, neueste zuerst) ───────────────────
const nachtraege = computed<{ name: string; timestamp: string; text: string }[]>(() =>
  ((props.data?.history ?? []) as any[])
...
async function submitNachtrag() {
  ...
  try {
    await client.post(`/tickets/${props.data.id}/nachtrag`, { text })
    nachtragText.value = ''
...
<template>
  <div class="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-5">
    ...
    <aside class="space-y-4">
      ...
      <!-- Edit-Lock: wer bearbeitet gerade? (read-only) -->
      ...
      <div class="space-y-5">
        ...
        <div v-if="data.status === 'archived'"
          ...
          <div v-if="showNachtrag"
            ...
            <textarea v-model="nachtragText" rows="4"
        ...
        <!-- Auftragsinhalt -->
        <component v-if="contentPanel" :is="contentPanel" :description="data.description" />
...
export default { components: { DescriptionRenderer } }
</script>
