...
/**
 * Klassischer Prozess-Editor (bisherige Fassung) – EIN Weg je Absicht.
 *
 * Bleibt während des Umbaus auf den neuen Editor (v2) unverändert erreichbar;
 * der Umschalter im Kopf wechselt zur neuen Fassung. Sonst identisch zur früheren
 * ProcessEditorView.
 *
 * Tabs:
...
/** Zum neuen Editor wechseln (bei ungespeicherten Änderungen nachfragen). */
function switchToV2() {
  if (ed.dirty.value
      && !confirm('Zum neuen Editor wechseln? Nicht gespeicherte Änderungen gehen verloren.')) return
  editorFlag.set(true)
}
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
      ...
      <div class="flex items-start justify-between gap-4 flex-wrap mb-4">
        ...
        <div class="flex items-center gap-2">
          <!-- Umschalter auf den neuen Editor (Beta) -->
          <button type="button" @click="switchToV2"
                  class="text-xs px-2.5 py-1 rounded-lg border border-[#3EAAB8]/40 text-[#3EAAB8]
                         hover:bg-[#3EAAB8]/10 transition" title="Neuen Editor ausprobieren">
            ✨ Neuer Editor
          </button>
